...
async function criarOcorrencia(campos) {
  ...
  if (USANDO_POSTGRES) {
    await pool.query(
      `insert into ocorrencias (id, data, loja, regional, descricao, "valorRecuperado", quadrilha, nome, inibido, "apoioCopia", monitoramento, "criadoEm")
...
app.post("/api/ocorrencias", checarSenha, async (req, res) => {
  const { data, loja, regional, descricao, valorRecuperado, quadrilha, nome, inibido, apoioCopia, monitoramento } = req.body;
